...
void philosopher()
{
	...
	int public_id = 0;
	int id = public_id++;
	sem_t *fork1 = (id + 1 < NUM) ? forks[id]     : forks[0];
	sem_t *fork2 = (id + 1 < NUM) ? forks[id + 1] : forks[id];
	cnt_t delay, i;

	for (;;)
	{
		LED[id] = 0;        // Philosopher is starving (led is off)
		sem_wait(fork1);    // Philosopher is waiting for the first fork
		sem_wait(fork2);    // Philosopher is waiting for the second fork
		delay = get_delay();
		LED[id] = 0;        // Philosopher is eating (led is blinking)
		for (i = 0; i < delay; i += SEC / 10) { LED[id]++; tsk_delay(SEC / 10); }
		sem_give(fork2);    // Philosopher puts down the second fork
		sem_give(fork1);    // Philosopher puts down the first fork
...
int main()
{
	...
	for (i = 0; i < NUM; i++)
	{
		forks[i] = sem_create(1, semBinary);

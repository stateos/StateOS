...
static tsk_t IDLE;
static tsk_t MAIN = { .id=ID_READY, .next=&IDLE, .prev=&IDLE, .prio=MAINPRIO, .top=MAIN_SP };                        // main task
static tsk_t IDLE = { .id=ID_IDLE,  .next=&MAIN, .prev=&MAIN, .prio=IDLEPRIO, .top=IDLE_SP, .state=port_idle_hook }; // idle task and tasks queue
